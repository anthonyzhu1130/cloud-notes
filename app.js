(function () {
  ...
  function safeFileName(name) {
    ...
    if (n.length > 120) {
      ...
      if (dot > 3) n = n.slice(0, 100) + n.slice(dot);
      else n = n.slice(0, 120);
    }
    return n;
  }
  ...
  function contentsUrl(path) {
    var clean = String(path || '').split('/').filter(Boolean).map(encodeURIComponent).join('/');
  ...
  function describeError(status, detail) {
    ...
    switch (status) {
      ...
      case 401: return '身份验证失败，请检查 Token 是否正确或已过期。' + tail;
      case 403: return /rate limit/i.test(detail) ? 'GitHub API 请求次数受限，请稍后再试。' + tail : '权限不足（403）：请检查 Token 权限。' + tail;
      case 404: return '未找到资源（404）：请检查用户名、仓库名、分支名、文件路径和 Token 权限。' + tail;
      case 409: return '远端文件已变化，请重新加载后再编辑。' + tail;
      case 413: return '请求内容过大（413）：请压缩图片或选择更小的文件。' + tail;
      case 422: return '请求参数错误（422）：请检查 sha、分支、文件路径。' + tail;
  ...
  function ghFetch(url, options) {
    ...
    return fetch(url, init).then(function (res) {
      ...
      throw new AppError('网络请求失败，请检查网络连接。', 0, String((err && err.message) || err));
    });
  }
  ...
  function parseNotesJson(text, sha) {
    ...
    try { json = JSON.parse(text); } catch (e) {
      throw new AppError('远端 notes.json 不是合法 JSON。已停止操作，请先手动修复。', 0);
    }
  ...
  function fetchRemoteNotes(token) {
    ...
    return ghFetch(url, { token: token || state.token || null }).then(function (res) {
      ...
      if (res.content && res.encoding === 'base64') {
        return parseNotesJson(base64ToUtf8(res.content), res.sha);
      }
      if (res.download_url) {
        return fetch(res.download_url + '?t=' + Date.now(), { cache: 'no-store' }).then(function (r) {
  ...
  function normalizeData(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new AppError('notes.json 结构不正确：顶层必须是对象。', 0);
    if (raw.categories !== undefined && !Array.isArray(raw.categories)) throw new AppError('notes.json 结构不正确：categories 必须是数组。', 0);
    if (raw.notes !== undefined && !Array.isArray(raw.notes)) throw new AppError('notes.json 结构不正确：notes 必须是数组。', 0);
  ...
  function commitData(nextData, message) {
    if (!state.token) return Promise.reject(new AppError('当前不是编辑模式，无法写入 GitHub。', 401));
    return fetchRemoteNotes().catch(function (err) {
      if (err && err.status === 404) return null;
      throw err;
    }).then(function (remote) {
      if (remote && state.remoteUpdatedAt && remote.json.updatedAt && remote.json.updatedAt !== state.remoteUpdatedAt) {
        throw new AppError('远端文件已变化，可能有其他设备在编辑。请重新加载后再编辑。', 409);
      }
  ...
  function putRepoFile(path, base64Content, message) {
    return ghFetch(contentsUrl(path) + '?ref=' + encodeURIComponent(CONFIG.BRANCH || 'main') + '&t=' + Date.now(), { token: state.token }).catch(function (err) {
      if (err && err.status === 404) return null;
      throw err;
  ...
  function deleteRepoFile(path, message) {
    return ghFetch(contentsUrl(path) + '?ref=' + encodeURIComponent(CONFIG.BRANCH || 'main') + '&t=' + Date.now(), { token: state.token }).then(function (info) {
      ...
      return ghFetch(contentsUrl(path), {
        ...
        body: { message: message, sha: info.sha, branch: CONFIG.BRANCH || 'main' }
      });
    });
  }
  ...
  function uploadAttachment(kind, noteId, item) {
    ...
    var safe = safeFileName(item.file.name);
    var storedName = item.id + '-' + safe;
    var path = root + '/' + noteId + '/' + storedName;
    return readFileAsArrayBuffer(item.file).then(function (buffer) {
      var base64 = arrayBufferToBase64(buffer);
      var message = (isImage ? '上传图片：' : '上传附件：') + item.file.name;
      return putRepoFile(path, base64, message).then(function (res) {
        var sha = (res && res.content && res.content.sha) ? res.content.sha : '';
        var meta = {
          id: item.id, originalName: item.file.name, storedName: storedName, path: path,
  ...
  function pausePomodoro() {
    if (toolState.pomodoroTimer) { clearInterval(toolState.pomodoroTimer); toolState.pomodoroTimer = null; }
  }
  ...
  function updateTimerDisplay() {
    var el = byId('timerDisplay');
    if (el) el.textContent = pad(Math.floor(toolState.timerRemaining / 60)) + ':' + pad(toolState.timerRemaining % 60);
  }

  function startTimer() {
    if (toolState.timerInterval) return;
    if (toolState.timerRemaining <= 0) {
      var inp = byId('timerMinutes');
  ...
  function pauseTimer() {
    if (toolState.timerInterval) { clearInterval(toolState.timerInterval); toolState.timerInterval = null; }
  }
  ...
  function noteMatches(note, keyword) {
    ...
    (note.attachments || []).forEach(function (f) {
      if (String(f.originalName || '').toLowerCase().indexOf(keyword) !== -1) hit = true;
    });
    return hit;
  }
  ...
  function setHighlightedText(el, text, keyword) {
    ...
    var index = 0, pos = lowerSource.indexOf(lowerKw, index), guard = 0;
  ...
  function renderSidebar() {
    var container = byId('categoryNav');
    if (!container) return;
    ...
    if (groups.length === 0) {
      ...
      p2.textContent = keyword ? '未找到匹配的笔记' : '暂无目录';
    ...
    groups.forEach(function (group) {
      ...
      countEl.textContent = String(group.notes.length);
  ...
  function renderNoteView(noteId) {
    ...
    var gallery = byId('noteViewGallery');
    gallery.textContent = '';
    (note.images || []).forEach(function (img) {
      var fig = document.createElement('figure');
      fig.className = 'note-figure';
      var a = document.createElement('a');
      a.href = assetUrl(img.path);
      a.target = '_blank';
      a.rel = 'noopener noreferrer';
      var im = document.createElement('img');
      im.src = assetUrl(img.path);
      im.alt = img.description || img.originalName;
      a.appendChild(im);
      fig.appendChild(a);
      var cap = document.createElement('figcaption');
      cap.textContent = img.description || img.originalName;
      fig.appendChild(cap);
      gallery.appendChild(fig);
    });
    var atts = byId('noteViewAttachments');
    atts.textContent = '';
    (note.attachments || []).forEach(function (f) {
      var li = document.createElement('li');
      var a = document.createElement('a');
  ...
  function render() {
    ...
    if (state.currentView === 'dashboard') renderDashboard();
      ...
      byId('pomodoroView').classList.remove('hidden');
      updatePomodoroDisplay();
    } else if (state.currentView === 'timer') {
      byId('timerView').classList.remove('hidden');
      updateTimerDisplay();
    }
  }
  ...
  function fillCategorySelect(select, selectedId) {
    ...
    cats.forEach(function (c) {
      ...
      o2.value = c.id;
      o2.textContent = c.name + (c.marked ? '（重点）' : '');
      if (c.id === selectedId) o2.selected = true;
  ...
  function verifyToken(token) {
    ...
    return ghFetch(apiBase(), { token: token }).then(function (info) {
      ...
      return fetchRemoteNotes(token).catch(function (err) {
        if (err && err.status === 404) throw new AppError('可访问仓库，但读取 notes.json 失败。请检查文件、分支和 Token 权限。', 404);
        throw err;
      });
    });
  }
  ...
  function confirmEnterEditMode() {
    var input = byId('tokenInput');
    var errEl = byId('tokenError');
    var btn = byId('btnTokenConfirm');
  ...
  function renderEditExistingMedia() {
    var imgBox = byId('editNoteExistingImages');
    var fileBox = byId('editNoteExistingFiles');
    imgBox.textContent = '';
    fileBox.textContent = '';
    ...
    if (images.length === 0) {
      var p = document.createElement('p'); p.className = 'empty-hint'; p.textContent = '暂无图片'; imgBox.appendChild(p);
    } else {
      images.forEach(function (img) {
        var row = document.createElement('div'); row.className = 'pending-item';
        var im = document.createElement('img'); im.src = assetUrl(img.path); im.alt = img.originalName || '';
        row.appendChild(im);
        var name = document.createElement('span'); name.className = 'pending-name';
        name.textContent = (img.originalName || '') + '（' + formatSize(img.size) + '）';
        row.appendChild(name);
        var del = document.createElement('button');
        del.type = 'button'; del.className = 'btn btn-small btn-danger'; del.textContent = '删除';
    ...
    if (files.length === 0) {
      var p2 = document.createElement('p'); p2.className = 'empty-hint'; p2.textContent = '暂无附件'; fileBox.appendChild(p2);
    } else {
      files.forEach(function (f) {
        var row = document.createElement('div'); row.className = 'pending-item';
        var name = document.createElement('span'); name.className = 'pending-name';
        name.textContent = (f.originalName || '') + '（' + formatSize(f.size) + '）';
        row.appendChild(name);
        var del = document.createElement('button');
        del.type = 'button'; del.className = 'btn btn-small btn-danger'; del.textContent = '删除';
        del.addEventListener('click', function () {
  ...
  function renderPendingList(container, items, kind, onRemove) {
    ...
    items.forEach(function (item) {
      var row = document.createElement('div'); row.className = 'pending-item';
      if (kind === 'image') {
        var im = document.createElement('img'); im.src = item.previewUrl; im.alt = item.file.name;
        row.appendChild(im);
      }
      var nameEl = document.createElement('span');
      ...
      if (kind === 'image') {
        ...
        label.textContent = '图片说明';
        var id = 'desc-' + item.id;
        label.setAttribute('for', id);
        var inp = document.createElement('input');
        inp.type = 'text'; inp.id = id; inp.value = item.description || ''; inp.maxLength = 200;
        inp.addEventListener('input', function () { item.description = inp.value; });
        descWrap.appendChild(label);
        descWrap.appendChild(inp);
      ...
      var del = document.createElement('button');
      del.type = 'button'; del.className = 'btn btn-small btn-danger'; del.textContent = '移除';
  ...
  function handleEditNoteImageSelect(files) {
    ...
    Array.prototype.forEach.call(files, function (file) {
      ...
      editNoteCtx.images.push({
        id: newId('image'), file: file, description: '',
        previewUrl: URL.createObjectURL(file), status: 'waiting', statusText: '等待上传'
      });
    });
  ...
  function saveEditNote() {
    ...
    var saveBtn = byId('btnSaveEditNote');
    var errEl = byId('editNoteError');
    errEl.classList.add('hidden');
    errEl.textContent = '';
    ...
    if (!content.trim() && keptImages.length === 0 && editNoteCtx.images.length === 0 && keptFiles.length === 0 && editNoteCtx.files.length === 0) {
      errEl.textContent = '标题、图片和附件不能全部为空。';
      errEl.classList.remove('hidden');
      return;
    }
    ...
    var uploadedImages = [];
    var uploadedFiles = [];
    var reallyRemovedImages = [];
    var reallyRemovedFiles = [];
    var chain = Promise.resolve();
    editNoteCtx.images.forEach(function (item) {
      chain = chain.then(function () {
        updatePendingStatus(item, 'uploading');
        return uploadAttachment('image', noteId, item).then(function (meta) {
          uploadedImages.push(meta);
          updatePendingStatus(item, 'done');
        }).catch(function (err) {
          updatePendingStatus(item, 'error');
          throw new AppError('图片「' + item.file.name + '」上传失败：' + ((err && err.message) || ''), err && err.status);
        });
      });
    });
    editNoteCtx.files.forEach(function (item) {
      chain = chain.then(function () {
        updatePendingStatus(item, 'uploading');
        return uploadAttachment('file', noteId, item).then(function (meta) {
          uploadedFiles.push(meta);
          updatePendingStatus(item, 'done');
        }).catch(function (err) {
          updatePendingStatus(item, 'error');
          throw new AppError('附件「' + item.file.name + '」上传失败：' + ((err && err.message) || ''), err && err.status);
        });
      });
    });
    if (!isNew) {
      (note.images || []).forEach(function (img) {
        ...
        chain = chain.then(function () {
          return deleteRepoFile(img.path, '删除图片：' + (img.originalName || '')).then(function () {
            reallyRemovedImages.push(img.id);
          }).catch(function (err) {
            if (err && err.status === 404) reallyRemovedImages.push(img.id);
            else warnings.push('图片删除失败：' + img.path);
          });
        });
      });
      (note.attachments || []).forEach(function (f) {
        ...
        chain = chain.then(function () {
          return deleteRepoFile(f.path, '删除附件：' + (f.originalName || '')).then(function () {
            reallyRemovedFiles.push(f.id);
          }).catch(function (err) {
            if (err && err.status === 404) reallyRemovedFiles.push(f.id);
            else warnings.push('附件删除失败：' + f.path);
          });
        });
      });
    }
    chain.then(function () {
      ...
      if (isNew) {
        next.notes.push({
          id: noteId, categoryId: categoryId, title: title, content: content, marked: marked,
          collapsed: false, images: uploadedImages, attachments: uploadedFiles,
  ...
  function addCategory() {
    ...
    var input = byId('newCategoryName');
    if (!input) return;
    var name = String(input.value || '').trim();
    if (!name) { notify('error', '目录名称不能为空。', 5000); return; }
    if (state.data.categories.some(function (c) { return c.name === name; })) {
      notify('error', '已存在同名目录。', 5000);
      return;
    }
    var nowIso = new Date().toISOString();
    var next = deepClone(state.data);
    next.categories.push({
      id: newId('category'), name: name, marked: false, collapsed: false,
      createdAt: nowIso, updatedAt: nowIso
    });
    var tip = notify('info', '正在保存…', 0);
    commitData(next, '新增目录：' + name).then(function () {
      input.value = '';
      tip.remove();
      ...
      tip.remove();
      notify('error', (err && err.message) || '保存失败。', 12000);
    });
  }
  ...
  function toggleNoteMarked(id) {
    ...
    var tip = notify('info', '正在保存…', 0);
    commitData(next, (note.marked ? '设为重点笔记：' : '取消重点笔记：') + note.title).then(function () {
      tip.remove();
      notify('success', '已保存到 GitHub。', 5000);
      render();
    }).catch(function (err) {
  ...
  function deleteNote(id) {
    ...
    if (!note) return;
    var imgCount = (note.images || []).length;
    var fileCount = (note.attachments || []).length;
    ...
    askConfirm({ title: '删除笔记', message: message, confirmText: '确认删除' }).then(function (ok) {
      ...
      (note.images || []).forEach(function (img) {
        if (!img.path) return;
        chain = chain.then(function () {
          return deleteRepoFile(img.path, '删除图片：' + (img.originalName || '')).catch(function (err) {
            if (!err || err.status !== 404) warnings.push('图片删除失败：' + img.path);
          });
        });
      });
      (note.attachments || []).forEach(function (f) {
        if (!f.path) return;
        chain = chain.then(function () {
          return deleteRepoFile(f.path, '删除附件：' + (f.originalName || '')).catch(function (err) {
            if (!err || err.status !== 404) warnings.push('附件删除失败：' + f.path);
          });
        });
      });
      chain.then(function () {
        ...
        if (state.currentView === 'note' && state.currentNoteId === id) {
        ...
        tip.remove();
        notify('error', (err && err.message) || '删除失败。', 12000);
      });
    });
  }
  ...
  function handleImportFile(file) {
    ...
    reader.onload = function () {
      ...
      catch (e) { notify('error', '导入失败：不是合法 JSON。', 10000); return; }
      var summaryEl = byId('importSummary');
      var errEl = byId('importError');
  ...
  function confirmImport() {
    ...
    commitData(importCandidate, '导入并覆盖 notes.json').then(function () {
      ...
      tip.remove();
      var errEl = byId('importError');
      errEl.textContent = (err && err.message) || '导入失败。';
      errEl.classList.remove('hidden');
  ...
  function loadScriptOnce(src) {
    return new Promise(function (resolve, reject) {
      ...
      s.onerror = function () { if (s.parentNode) s.parentNode.removeChild(s); reject(new Error('脚本加载失败')); };
  ...
  function loadMammoth() {
    ...
    mammothPromise = (function () {
      ...
      return p.catch(function () {
        mammothPromise = null;
        throw new AppError('无法加载 Word 解析库，请检查网络。', 0);
      });
    })();
  ...
  function extractBlocksFromHtml(html) {
    ...
    function walk(node) {
      ...
      if (tag === 'p') {
        ...
        var t2 = textOf(node);
        if (t2) blocks.push({ type: 'paragraph', text: t2 });
        else if (imgs.length) blocks.push({ type: 'paragraph', text: '[图片：Word 中的图片不会自动导入]' });
        return;
      }
      if (tag === 'ul' || tag === 'ol') {
        var items = [];
  ...
  function buildWordPreview(blocks, mode, fileName) {
    if (mode === 'single') {
      ...
      return { mode: 'single', singleNote: { title: fileName || '导入的 Word', content: lines.join('\n\n') } };
    }
    var categories = [];
    var currentCat = null, currentNoteTitle = null, buffer = [];
    function ensureCat(name) {
      var c = categories.find(function (x) { return x.name === name; });
      if (!c) { c = { name: name, notes: [] }; categories.push(c); }
      return c;
    }
    function flush() {
      if (currentCat && (currentNoteTitle || buffer.length)) {
        currentCat.notes.push({ title: currentNoteTitle || currentCat.name, content: buffer.join('\n\n').trim() });
      }
      buffer = [];
    ...
    blocks.forEach(function (b) {
      if (b.type === 'heading') {
        if (b.level === 1) { flush(); currentCat = ensureCat(b.text); }
          ...
          flush();
          if (!currentCat) currentCat = ensureCat(fileName || 'Word 导入');
          currentNoteTitle = b.text;
        } else {
          if (currentNoteTitle) buffer.push('【' + b.text + '】');
        }
  ...
  function refreshWordPreview() {
    ...
    var singleRow = byId('wordImportSingleRow');
    if (singleRow) singleRow.style.display = (mode === 'single') ? '' : 'none';
    var preview = buildWordPreview(wordCtx.blocks, mode, wordCtx.fileName);
    wordCtx.preview = preview;
    var summaryEl = byId('wordImportSummary');
    var errEl = byId('wordImportError');
    errEl.classList.add('hidden');
    if (mode === 'h1-h2') {
      ...
      preview.categories.forEach(function (c) {
        s += '📁 ' + c.name + '（' + c.notes.length + ' 篇）\n';
  ...
  function handleWordFile(file) {
    ...
    loadMammoth().then(function () {
      ...
      loading.remove();
      notify('error', (err && err.message) || '解析 Word 失败。', 12000);
    });
  }
  ...
  function executeWordImport() {
    ...
    if (!preview) return;
    var btn = byId('btnWordImportConfirm');
    ...
    try {
      ...
      if (preview.mode === 'single') {
        ...
        next.notes.push({
          id: newId('note'), categoryId: categoryId, title: preview.singleNote.title,
          content: preview.singleNote.content, marked: false, collapsed: false,
          images: [], attachments: [], createdAt: nowIso, updatedAt: nowIso
        });
      } else {
        preview.categories.forEach(function (cp) {
          var cat = next.categories.find(function (c) { return c.name === cp.name; });
          if (!cat) {
            cat = { id: newId('category'), name: cp.name, marked: false, collapsed: false, createdAt: nowIso, updatedAt: nowIso };
            next.categories.push(cat);
          }
          cp.notes.forEach(function (np) {
            next.notes.push({
              id: newId('note'), categoryId: cat.id, title: np.title, content: np.content,
  ...
  function bindEvents() {
    byId('btnEnterEdit').addEventListener('click', enterEditMode);
    byId('btnExitEdit').addEventListener('click', exitEditMode);
    byId('btnToggleToken').addEventListener('click', function () {
      var inp = byId('tokenInput');
      var btn = byId('btnToggleToken');
      if (inp.type === 'password') { inp.type = 'text'; btn.textContent = '隐藏'; }
      else { inp.type = 'password'; btn.textContent = '显示'; }
    });
    byId('btnTokenConfirm').addEventListener('click', confirmEnterEditMode);
    byId('btnTokenCancel').addEventListener('click', function () { byId('tokenInput').value = ''; hideModal('tokenModal'); });
    byId('tokenInput').addEventListener('keydown', function (ev) {
      if (ev.key === 'Enter') { ev.preventDefault(); confirmEnterEditMode(); }
    });

    byId('confirmOkBtn').addEventListener('click', function () { resolveConfirm(true); });
    byId('confirmCancelBtn').addEventListener('click', function () { resolveConfirm(false); });

    byId('btnToggleSidebar').addEventListener('click', function () {
    ...
    byId('btnBackFromPomodoro').addEventListener('click', function () { state.currentView = 'dashboard'; render(); });
    byId('btnBackFromTimer').addEventListener('click', function () { state.currentView = 'dashboard'; render(); });
    ...
    byId('btnClearSearch').addEventListener('click', function () {
      byId('searchInput').value = '';
      state.searchKeyword = '';
      renderSidebar();
    });

    byId('btnQuickAddNote').addEventListener('click', function () { openEditNote(null); });
    byId('btnQuickReload').addEventListener('click', function () { loadRemoteData({}); });
    byId('btnQuickExport').addEventListener('click', exportBackup);
    byId('btnImportWordDash').addEventListener('click', function () { byId('wordFileInput').click(); });

    byId('btnBackToDash').addEventListener('click', function () {
      state.currentView = 'dashboard';
      state.currentNoteId = null;
      render();
    });
    byId('btnEditCurrentNote').addEventListener('click', function () {
      if (state.currentNoteId) openEditNote(state.currentNoteId);
    });
    byId('btnDeleteCurrentNote').addEventListener('click', function () {
      if (state.currentNoteId) deleteNote(state.currentNoteId);
    });
    byId('btnToggleMarkCurrentNote').addEventListener('click', function () {
      if (state.currentNoteId) toggleNoteMarked(state.currentNoteId);
    });

    var btnAddCat = byId('btnAddCategory');
    if (btnAddCat) btnAddCat.addEventListener('click', addCategory);
    var newCatName = byId('newCategoryName');
    if (newCatName) {
      newCatName.addEventListener('keydown', function (ev) {
        if (ev.key === 'Enter') { ev.preventDefault(); addCategory(); }
      });
    }

    byId('editNoteImages').addEventListener('change', function (ev) {
      var files = ev.target.files; ev.target.value = '';
      if (files && files.length) handleEditNoteImageSelect(files);
    });
    byId('editNoteFiles').addEventListener('change', function (ev) {
      var files = ev.target.files; ev.target.value = '';
      if (files && files.length) handleEditNoteFileSelect(files);
    });
    byId('btnSaveEditNote').addEventListener('click', saveEditNote);
    byId('btnCancelEditNote').addEventListener('click', function () { closeEditNoteModal(); });
    ...
    byId('btnImportConfirm').addEventListener('click', confirmImport);
    byId('btnImportCancel').addEventListener('click', function () {
      importCandidate = null;
      hideModal('importModal');
    });

    byId('wordFileInput').addEventListener('change', function (ev) {
      var f = ev.target.files && ev.target.files[0];
      ev.target.value = '';
      if (f) handleWordFile(f);
    });
    byId('wordImportMode').addEventListener('change', refreshWordPreview);
    byId('wordImportSingleCategory').addEventListener('change', refreshWordPreview);
    byId('btnWordImportConfirm').addEventListener('click', executeWordImport);
    byId('btnWordImportCancel').addEventListener('click', function () {
      wordCtx.blocks = null; wordCtx.preview = null; wordCtx.fileName = '';
      hideModal('wordImportModal');
    });
    ...
    document.addEventListener('keydown', function (ev) {
      ...
      if (!byId('importModal').classList.contains('hidden')) { importCandidate = null; hideModal('importModal'); return; }
